...
def main():
    ...
    parser = argparse.ArgumentParser(prog='whisk', description='All in One action deep learning')
    parser.add_argument('file', type=str, help='Relative path to file containing dataset and model')
    parser.add_argument('-m', '--memory', type=int, help='Action memory. \nDefault value is 8196.')
    parser.add_argument('-b', '--batch', type=int, help='Size of data batch. \nDefault value is 64.')
...
def build(params):
    ...
    timeout = 60000000
    metadata = json.dumps({"model" : file_content['model']['name'], "dataset" : dataset, "batch": batch_size, "epochs": epochs, "features": params["features"]})
    #import pdb
    #pdb.set_trace()
    ...
    action_code = "wsk -i action update deep/learning learning.py --docker {2} --memory {0} --timeout {1}".format(memory, timeout, image)
    post(action_code)
...
def post(command):
    ...
    reply = subprocess.run(['/bin/bash', '-c', command], stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.decode().strip()
    if "error" in reply:
        print('Error:', reply)
    else:
        print('Result:', reply)
