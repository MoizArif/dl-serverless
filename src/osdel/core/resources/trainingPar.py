...
def main(args):
    ...
    class models():
        def custom05(shape, label):
            # Custom model with 5 layers
            model = tf.keras.Sequential([
                tf.keras.layers.Conv2D(32, 3, activation='relu', input_shape=(shape[0], shape[1], shape[2])),
        ...
        def vgg16(shape, label):
            from tensorflow.keras.applications.vgg16 import VGG16
            # Using vgg model
            vgg_model = VGG16(input_shape=(shape[0], shape[1], shape[2]), # Shape of our images
                                include_top = False,
                                weights = 'imagenet')
            # Flatten a single dimension
            x = tf.keras.layers.Flatten()(vgg_model.output)
            # Add a fully connected layer of 64 hidden
            x = tf.keras.layers.Dense(64, activation='relu')(x)
            # Add a final layer with the number of output channel corresponding to the dataset
            x = tf.keras.layers.Dense(label, activation='softmax')(x)
        ...
        def resnet50(shape, label):
            from tensorflow.keras.applications import ResNet50
            # Build out of a Sequential model configuration
            model = tf.keras.models.Sequential()
            # Include resnet model from tensorflow
            model.add(ResNet50(input_shape=(shape[0], shape[1], shape[2]), include_top=False, weights='imagenet', pooling='max'))
            # Flatten a single dimension
        ...
        def mobilenet(shape, label):
            from tensorflow.keras.applications import MobileNet
            # Build out of a Sequential model configuration
            model = tf.keras.models.Sequential()
            # Include resnet model from tensorflow
            model.add(MobileNet(input_shape=(shape[0], shape[1], shape[2]), include_top=False, weights='imagenet', pooling='max'))
            # Flatten a single dimension
            model.add(tf.keras.layers.Flatten())
            # Add a fully connected layer of 64 hidden
            model.add(tf.keras.layers.Dense(64, activation='relu'))
    ...
    train_features = conv_base.predict(np.array(train_X), batch_size=batch_size * strategy.num_replicas_in_sync, verbose=2)
    ...
    print("REDIS IS AVAILABLE AT {0}:{1}".format(parsed.hostname,parsed.port))
