...
class Scheduler():
    ...
    def estimateMemory(self, job=0, number_of_action=1):
        ...
        print('data: {0} | fit: {1} | package: {2} | model: {3}'.format(
            data, fit_estimate, package_size, model_size))
    ...
    def getTrainingMemory(self, number_of_action):
        ...
        total_memory = (activation_memory * 2 * (self.batch /
                                                 number_of_action)) + params_memory + misc_memory * self.epoch
    ...
    def __parameterCount(self):
        model = Model(self.model, self.dataset_shape,
